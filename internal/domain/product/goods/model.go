...
type GoodsRequest struct {
	...
	Status     models.StatusEnum `json:"status" enums:"unlisted,listed,inactive"` // 商品状态：未上架 或 已上架 或 已下架
...
type GoodsUpdateRequest struct {
	...
	Images     []string          `json:"images"`                                  // 商品图片 URL 列表
	CategoryID string            `bson:"category_id" json:"categoryId"`           // 所属分类 ID
	Specs      []GoodsSpec       `json:"specs" validate:"required"`               // 商品规格列表
	Status     models.StatusEnum `json:"status" enums:"unlisted,listed,inactive"` // 商品状态：未上架 或 已上架 或 已下架
	Tags       []string          `json:"tags"`                                    // 商品标签
}
...
type GoodsSpec struct {
	SpecID   string      `json:"specId"`                     // 规格 ID
	SpecName i18n.Locale `json:"specName"`                   // 规格名称（多语言）
