...
// 根据角色名查找
func (r *RoleRepo) FindByRoleName(ctx context.Context, name string) (*model.RoleResponse, error) {
	var role model.RoleResponse
...
func (r *RoleRepo) UpdateRole(ctx context.Context, data *model.RoleUpdateRequest) (*model.RoleResponse, error) {
	...
	_, err := r.collection.UpdateOne(ctx, bson.M{"role_id": data.RoleId}, bson.M{"$set": update})
...
func (r *RoleRepo) DeleteRole(ctx context.Context, roleId string) error {
	_, err := r.collection.DeleteOne(ctx, bson.M{"role_id": roleId})
